...
\subsection{Avoiding keyval option classes between classes and packages}

In \LaTeX{} News~35~\cite{39:ltnews35} we introduced keyval option processing
...
every package. However, with keyvals, the likelihood of a name class between a
...
uses an option which should be global (for example \texttt{draft}), a
new key property \texttt{.scope} has been added. This can then be set to
